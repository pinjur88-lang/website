...
CREATE TABLE IF NOT EXISTS donations (
    ...
    -- "Guests" type this in manually
    donor_email text,
    -- Important for receipt/upsell
    message text,
    -- "In Memory Of..."
    project_id uuid REFERENCES projects(id) ON DELETE CASCADE,
    user_id uuid REFERENCES auth.users(id),
    -- NULLABLE (Crucial for Guests)
...
CREATE POLICY "Public view projects" ON projects FOR
SELECT USING (true);
DROP POLICY IF EXISTS "Admins manage projects" ON projects;
CREATE POLICY "Admins manage projects" ON projects FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profiles
        WHERE id = (
...
CREATE POLICY "Public view non-anonymous donations" ON donations FOR
SELECT USING (is_anonymous = false);
...
CREATE POLICY "Everyone can donate" ON donations FOR
INSERT WITH CHECK (true);
-- 5. TRIGGER: Auto-update Project Current Amount
CREATE OR REPLACE FUNCTION update_project_amount() RETURNS TRIGGER AS $$ BEGIN
UPDATE projects
...
AFTER
INSERT ON donations FOR EACH ROW EXECUTE FUNCTION update_project_amount();
