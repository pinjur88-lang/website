...
ADD COLUMN IF NOT EXISTS full_name text; -- Alias for name if needed, or we just map it
...
BEGIN
  -- Extract Metadata
  meta_full_name := new.raw_user_meta_data->>'display_name'; -- supabase auth usually sends this
  ...
  SET 
    ...
    role = 'pending'; -- Reset role if they try to re-register? Maybe safer to keep existing.
  ...
  VALUES (
    new.email,
    COALESCE(meta_full_name, new.email), -- Fallback
    'pending',
  ...
  IF new.raw_user_meta_data->'family_members' IS NOT NULL THEN
    INSERT INTO public.family_members (head_of_household, full_name, date_of_birth, relationship)
    SELECT 
      new.id,
      (fm->>'full_name'),
  ...
  IF new.raw_user_meta_data->'company' IS NOT NULL THEN
    INSERT INTO public.companies (representative_id, company_name, company_oib, address)
    SELECT 
      new.id,
