...
ADD COLUMN IF NOT EXISTS full_name text,
    ...
    -- Can be null for foreign members
...
ADD COLUMN IF NOT EXISTS phone text,
    ...
    -- "Ime Oca" for identity
ADD COLUMN IF NOT EXISTS family_nickname text,
    -- "Obiteljski Nadimak"
ADD COLUMN IF NOT EXISTS house_number text,
    -- Link to map
ADD COLUMN IF NOT EXISTS consents jsonb DEFAULT '{"statute": false, "data": false, "marketing": false}';
-- GDPR
...
CREATE TABLE IF NOT EXISTS family_members (
    ...
    -- 'child', 'spouse', etc.
    legal_guardian text,
    -- Auto-filled from Head of Household's name
...
CREATE POLICY "Head of household manages family" ON family_members FOR ALL TO authenticated USING (
    (
        select auth.uid()
    ) = head_of_household
) WITH CHECK (
...
CREATE POLICY "Admins view all family members" ON family_members FOR
...
CREATE POLICY "Representative manages company" ON companies FOR ALL TO authenticated USING (
    (
        select auth.uid()
    ) = representative_id
) WITH CHECK (
...
CREATE POLICY "Admins view all companies" ON companies FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profiles
