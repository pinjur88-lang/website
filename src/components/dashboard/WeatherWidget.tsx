...
export default function WeatherWidget() {
    ...
    const getConditionText = (code: number) => {
        ...
        if (code === 1 || code === 2 || code === 3) return "Djelomično oblačno";
        if (code >= 51) return "Kiša";
        if (code >= 71) return "Snijeg";
        return "Sunčano";
    };
        ...
        <div className="bg-gradient-to-br from-blue-50 to-white p-6 rounded-lg border border-blue-100 shadow-sm">
            <div className="flex justify-between items-start mb-4">
                <div>
                    ...
                    <p className="text-xs text-stone-500">Trenutno Vrijeme</p>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs text-stone-600 bg-white/50 p-3 rounded-md">
                <div className="flex items-center gap-2">
                    <Wind size={14} />
                    <span>Vjetar: {current.wind_speed_10m} km/h</span>
                </div>
                <div className="flex items-center gap-2">
                    <Droplets size={14} />
                    <span>Vlaga: {current.relative_humidity_2m}%</span>
                </div>
            </div>
        </div>
    );
